 ...
 *  
 *  @lucene.internal */
public class NonHeapInputStream extends IndexInput implements Cloneable {
  static final int BUFFER_SIZE = NonHeapOutputStream.BUFFER_SIZE;

  private NonHeapFile file;
  private long length;
  ...
  @Deprecated
  public NonHeapInputStream(NonHeapFile f) throws IOException {
    this("anonymous", f);
  }

  public NonHeapInputStream(String name, NonHeapFile f) throws IOException {
    super("NonHeapInputStream(name=" + name + ")");
    file = f;
    length = file.length;
    if (length/BUFFER_SIZE >= Integer.MAX_VALUE) {
      throw new IOException("NonHeapInputStream too large length=" + length + ": " + name); 
    }
