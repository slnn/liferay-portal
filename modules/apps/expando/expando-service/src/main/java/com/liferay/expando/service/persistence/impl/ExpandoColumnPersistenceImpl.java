...
package com.liferay.expando.service.persistence.impl;
...
import com.liferay.expando.model.impl.ExpandoColumnModelImpl;
import com.liferay.expando.service.persistence.ExpandoColumnPersistence;
import com.liferay.expando.service.persistence.ExpandoColumnUtil;
import com.liferay.expando.service.persistence.impl.constants.ExpandoPersistenceConstants;
 ...
 * @generated
 */
@Component(service = ExpandoColumnPersistence.class)
public class ExpandoColumnPersistenceImpl
	...
	public List<ExpandoColumn> findByTableId(
		...
		if (useFinderCache && productionMode) {
			list = (List<ExpandoColumn>)finderCache.getResult(
	...
	public void clearCache() {
		...
		finderCache.clearCache(ExpandoColumnImpl.class);
	}
	...
	public void clearCache(ExpandoColumn expandoColumn) {
		entityCache.removeResult(ExpandoColumnImpl.class, expandoColumn);
	}
	...
	public void clearCache(Set<Serializable> primaryKeys) {
		...
		for (Serializable primaryKey : primaryKeys) {
			entityCache.removeResult(ExpandoColumnImpl.class, primaryKey);
		}
	}
	...
	protected void cacheUniqueFindersCache(
		...
		finderCache.putResult(_finderPathCountByT_N, args, Long.valueOf(1));
		finderCache.putResult(
	...
	protected ExpandoColumn removeImpl(ExpandoColumn expandoColumn) {
		...
		try {
			...
			if ((expandoColumn != null) &&
				ctPersistenceHelper.isRemove(expandoColumn)) {
	...
	public ExpandoColumn updateImpl(ExpandoColumn expandoColumn) {
		...
		try {
			...
			if (ctPersistenceHelper.isInsert(expandoColumn)) {
				if (!isNew) {
					session.evict(
	...
	public Map<Serializable, ExpandoColumn> fetchByPrimaryKeys(
		...
		if (ctPersistenceHelper.isProductionMode(ExpandoColumn.class)) {
	...
	@Override
	@Reference(
		target = ExpandoPersistenceConstants.SERVICE_CONFIGURATION_FILTER,
		unbind = "-"
	)
	...
	@Override
	@Reference(
		target = ExpandoPersistenceConstants.ORIGIN_BUNDLE_SYMBOLIC_NAME_FILTER,
		unbind = "-"
	)
	...
	private static final String _FILTER_SQL_SELECT_EXPANDOCOLUMN_WHERE =
		"SELECT DISTINCT {expandoColumn.*} FROM Expando_ExpandoColumn expandoColumn WHERE ";

	private static final String
		_FILTER_SQL_SELECT_EXPANDOCOLUMN_NO_INLINE_DISTINCT_WHERE_1 =
			"SELECT {Expando_ExpandoColumn.*} FROM (SELECT DISTINCT expandoColumn.columnId FROM Expando_ExpandoColumn expandoColumn WHERE ";

	private static final String
		_FILTER_SQL_SELECT_EXPANDOCOLUMN_NO_INLINE_DISTINCT_WHERE_2 =
			") TEMP_TABLE INNER JOIN Expando_ExpandoColumn ON TEMP_TABLE.columnId = Expando_ExpandoColumn.columnId";

	private static final String _FILTER_SQL_COUNT_EXPANDOCOLUMN_WHERE =
		"SELECT COUNT(DISTINCT expandoColumn.columnId) AS COUNT_VALUE FROM Expando_ExpandoColumn expandoColumn WHERE ";

	private static final String _FILTER_ENTITY_ALIAS = "expandoColumn";

	private static final String _FILTER_ENTITY_TABLE = "Expando_ExpandoColumn";

	private static final String _ORDER_BY_ENTITY_ALIAS = "expandoColumn.";

	private static final String _ORDER_BY_ENTITY_TABLE =
		"Expando_ExpandoColumn.";
