...
public class AssetEntryDocumentContributor
	...
	private AssetEntry _lookupAssetEntry(long classNameId, long classPK) {
		Map<Long, AssetEntry> indexedAssetEntries =
			ReindexCacheThreadLocal.getScopeReindexCache(
				...
				() -> _assetEntryLocalService.dslQueryCount(
					...
					false),
				count -> {
					Map<Long, AssetEntry> localIndexedAssetEntries =
						new HashMap<>();

					if (count == 0) {
						return localIndexedAssetEntries;
					}
		...
		if (indexedAssetEntries == null) {
			return _assetEntryLocalService.fetchEntry(classNameId, classPK);
		}

		return indexedAssetEntries.get(classPK);
	}
