...
public class GlowrootProxyServletTest {
	...
	public void testVisitGlowrootPage() throws Exception {
		...
		_visitGlowrootPage(_CUSTOMIZE_CONEXT_NAME);
	}
	...
	private void _visitGlowrootPage(String... targetBundleNameSuffixes)
		...
		if (targetBundleNameSuffixes.length == 1) {
			_targetBundleNameSuffix = targetBundleNameSuffixes[0];
		}
		...
		if (_targetBundleNameSuffix.equals(_CUSTOMIZE_CONEXT_NAME)) {
			...
			contextPath =
				StringPool.FORWARD_SLASH + _CUSTOMIZE_CONEXT_NAME + contextPath;
		}
		...
		if (!_startPortalWithGlowroot(processBuilder)) {
			...
			if (_log.isInfoEnabled()) {
				_log.info("Start to visit glowroot page with " + context + "!");
			}
	...
	private static String _targetBundleNameSuffix = "root";
