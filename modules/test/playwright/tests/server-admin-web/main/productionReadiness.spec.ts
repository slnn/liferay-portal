...
test('LPD-87225 - Production Readiness dashboard renders summary and tables and passes a11y review.', async ({
	...
	for (const label of ['Passed', 'Failed', 'Ignored'] as const) {
		...
		expect(await productionReadinessPage.summaryCount(label)).toBeGreaterThanOrEqual(0);
	}

	expect(await productionReadinessPage.categoryPanelButton().count()).toBeGreaterThan(0);
...
test('LPD-87225 - Ignoring a rule updates the summary counts and respects the Show Ignored filter.', async ({
	...
	const ignoredBaseline = await productionReadinessPage.summaryCount('Ignored');

	await productionReadinessPage.toggleIgnore(TARGET_RULE_KEY);

	await expect(productionReadinessPage.ruleToggle(TARGET_RULE_KEY)).toBeChecked();

	expect(await productionReadinessPage.summaryCount('Passed')).toBe(passedBaseline);
	expect(await productionReadinessPage.summaryCount('Failed')).toBe(failedBaseline - 1);
	expect(await productionReadinessPage.summaryCount('Ignored')).toBe(ignoredBaseline + 1);

	await productionReadinessPage.setFilter('Show Ignored', false);

	await expect(productionReadinessPage.ruleRow(TARGET_RULE_KEY)).toHaveCount(0);

	await productionReadinessPage.setFilter('Show Ignored', true);

	await expect(productionReadinessPage.ruleRow(TARGET_RULE_KEY)).toBeVisible();

	await productionReadinessPage.toggleIgnore(TARGET_RULE_KEY);

	await expect(productionReadinessPage.ruleToggle(TARGET_RULE_KEY)).not.toBeChecked();

	expect(await productionReadinessPage.summaryCount('Passed')).toBe(passedBaseline);
	expect(await productionReadinessPage.summaryCount('Failed')).toBe(failedBaseline);
	expect(await productionReadinessPage.summaryCount('Ignored')).toBe(ignoredBaseline);
});
