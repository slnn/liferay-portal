<#list dataFactory.newResourcePermissionModels("com.liferay.journal", guestGroupId) as resourcePermissionModel>
...
<#list dataFactory.getSequence(dataFactory.maxJournalArticlePageCount) as journalArticlePageCount>
	<#assign
		...
		layoutModel = dataFactory.newLayoutModel(guestGroupId, guestGroupId + "_journal_article_" + journalArticlePageCount, "", dataFactory.getJournalArticleLayoutColumn(portletIdPrefix))
	/>

	${csvFileWriter.write("layout", companyModel.webId + "," + layoutModel.friendlyURL + "\n")}

	<@insertLayout _layoutModel=layoutModel />

	<#list dataFactory.getSequence(dataFactory.maxJournalArticleCount) as journalArticleCount>
		<#assign journalArticleResourceModel = dataFactory.newJournalArticleResourceModel(guestGroupId) />
		...
		${dataFactory.toInsertSQL(dataFactory.newLayoutClassedModelUsageModel(guestGroupId, layoutModel.plid, portletIdPrefix + journalArticleCount, journalArticleResourceModel))}
